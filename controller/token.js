...
function tokenRenovation(id, userUpdate, nameOfOperation, res){
  var bool = false;
	Token.findOne({email: userUpdate.email}, (err, tokenStored) => {
		if(err){
			...
			if(!tokenStored){
				...
				Token.findOneAndUpdate({ email: tokenStored.email, generatedToken: tokenStored.generatedToken }, {email: id, generatedToken: newToken}, (err, tokenUpdate) => {
					if(err){
						res.status(500).send({message: 'Error en la petición'});
					}else{
						if(!tokenUpdate){
							...
							User.findOne({ email: id }, (err, userStored) => {
						    	if(err){
						    		res.status(500).send({message: 'Error en la petición'});
						    	}else{
						    		if(!userStored){
						    			...
						    			bool = true;
						    			if(nameOfOperation == 'updateAdministrator' || nameOfOperation == 'updateTUser' || nameOfOperation == 'resetPassword'){
